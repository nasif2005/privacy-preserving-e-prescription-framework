...
const (
	...
	samplePackageJSON  = "patient_wallet.json"
	groupKeyJSON       = "bbs_group_public_key.json"
	auditAuthorityJSON = "bbs_audit_authority.json"
)
...
func writeAuditAuthority(gmk *bbs04.GroupManagerKey, member *bbs04.MemberKey) error {
	xi1, xi2, gamma, certificate := gmk.Xi1.Bytes(), gmk.Xi2.Bytes(), gmk.Gamma.Bytes(), member.A.Bytes()
	encoded, err := json.MarshalIndent(SerializedAuditAuthority{
		Xi1: hex.EncodeToString(xi1[:]), Xi2: hex.EncodeToString(xi2[:]), Gamma: hex.EncodeToString(gamma[:]),
		Members: []AuditMemberRecord{{HPID: "HP-00001", LicenseNo: "LIC-CA-2026-00001", Organization: "Hospital-A", MemberCertificate: hex.EncodeToString(certificate[:])}},
	}, "", "  ")
	if err != nil {
		return err
	}
	return os.WriteFile(auditAuthorityJSON, encoded, 0o600)
}

func writeGroupPublicKey(gpk *bbs04.GroupPublicKey) error {
	g1, g2, h := gpk.G1.Bytes(), gpk.G2.Bytes(), gpk.H.Bytes()
	u, v, w := gpk.U.Bytes(), gpk.V.Bytes(), gpk.W.Bytes()
	encoded, err := json.MarshalIndent(SerializedGroupPublicKey{
		G1: hex.EncodeToString(g1[:]), G2: hex.EncodeToString(g2[:]),
		H: hex.EncodeToString(h[:]), U: hex.EncodeToString(u[:]),
		V: hex.EncodeToString(v[:]), W: hex.EncodeToString(w[:]),
	}, "", "  ")
	if err != nil {
		return err
	}
	return os.WriteFile(groupKeyJSON, encoded, 0o644)
}
...
var baseAttributes = []struct {
	...
	{Name: "holder", Value: "holder-public-key-001"},
	{Name: "rx_id", Value: "RX-2026-000001"},
	{Name: "patient_reference", Value: "PATIENT-REF-001"},
	{Name: "medication", Value: "MED-001"},
	{Name: "dosage", Value: "10-mg"},
}

// buildCredentialMessages creates the exact attribute map and scalar message
// vector consumed by Section 5.2.3. A six-message credential contains the five
// named prescription attributes and the expiry-commitment hash. Larger
// benchmark configurations append synthetic extension attributes before the
// final commitment-hash message.
func buildCredentialMessages(
	curveBBS *curves.PairingCurve,
	nAttr int,
	commitmentHash []byte,
) (map[string]string, []curves.Scalar) {
	attributes := make(map[string]string, nAttr)
	messages := make([]curves.Scalar, 0, nAttr)

	ordinaryCount := nAttr - 1
	for i := 0; i < ordinaryCount; i++ {
		var name, value string
		if i < len(baseAttributes) {
			name = baseAttributes[i].Name
			value = baseAttributes[i].Value
		} else {
			name = fmt.Sprintf("extension_attr_%02d", i-len(baseAttributes)+1)
			value = fmt.Sprintf("extension-value-%02d", i-len(baseAttributes)+1)
		}
...
func summarize(values []float64) timingSummary {
	...
	return timingSummary{MeanMS: mean, SDMS: standardDeviation, CV: coefficientOfVariation}
}
...
func runIssuanceExperiment(
...
) (float64, *PrescriptionPackage, error) {
	...
	attributes, messages := buildCredentialMessages(curveBBS, nAttr, commitmentHash[:])
	...
	if err = issuerPublicKey.Verify(bbsPlusSignature, messageGenerators, messages); err != nil {
		return 0, nil, fmt.Errorf("issued BBS+ credential verification failed: %w", err)
	}
	...
	packageOut := &PrescriptionPackage{
		Credential: Credential{
			ExpiryCommitment: hex.EncodeToString(commitmentBytes),
	...
	return elapsedMS, packageOut, nil
}
...
func main() {
	...
	if err = writeGroupPublicKey(groupPublicKey); err != nil {
		panic(err)
	}
	...
	if err = writeAuditAuthority(groupManagerKey, hpMemberKey); err != nil {
		panic(err)
	}
	...
	// Untimed runtime warm-up.
	for i := 0; i < warmupRuns; i++ {
		if _, _, err = runIssuanceExperiment(6, groupPublicKey, hpMemberKey); err != nil {
			panic(err)
		}
	}
	...
	if err = writer.Write([]string{
		"n_attr", "runs", "mean_ms", "stddev_ms", "cv_percent",
		"group_signature_bytes", "gomaxprocs",
	}); err != nil {
		panic(err)
	}

	fmt.Printf("%-8s %-12s %-12s %-10s\n", "N_attr", "Mean (ms)", "SD (ms)", "CV (%)")
	var finalSample *PrescriptionPackage
	for _, nAttr := range attributeCounts {
		timings := make([]float64, 0, runs)
		for run := 0; run < runs; run++ {
			elapsedMS, sample, runErr := runIssuanceExperiment(nAttr, groupPublicKey, hpMemberKey)
			if runErr != nil {
				panic(runErr)
			}
			timings = append(timings, elapsedMS)
			// Section 5.2.3 uses a fixed six-message credential: five named
			// prescription attributes plus the expiry-commitment hash.
			if nAttr == 6 && run == runs-1 {
				finalSample = sample
			}
		}
		...
		fmt.Printf("%-8d %-12.3f %-12.3f %-10.3f\n", nAttr, summary.MeanMS, summary.SDMS, summary.CV)
	}
	...
	fmt.Printf("Results: %s\nSample package: %s\nGroup public key: %s\nAudit authority: %s\n", resultsCSV, samplePackageJSON, groupKeyJSON, auditAuthorityJSON)
}
